...
The production GitHub Actions workflow stores Grafana Terraform state outside the checked-out repository under `/opt/lbpr/.terraform-state/grafana.tfstate` on the self-hosted runner. This keeps dashboard provisioning idempotent across deploys even when `actions/checkout` cleans the workspace.

Grafana dashboard provisioning is intentionally non-blocking in CI: the application deploy and health check run first, then dashboard apply is retried. If Grafana Cloud returns a transient API error, the workflow logs a warning instead of marking the application deploy as failed.

The folder UID defaults to `lbpr`. Keep `grafana_folder_uid` stable after the first successful apply. If an older `LBPR` folder already exists in Grafana with a different autogenerated UID and no Terraform state, either import that folder into the state path or remove/recreate it before applying.
