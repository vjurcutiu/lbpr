...
from core.grpc_errors import extract_grpc_error_info, format_grpc_error_for_user
...
async def _transcribe_streaming(
    *,
    ...
    try:
        ...
        user_err = format_grpc_error_for_user(e)
        raise HTTPException(status_code=502, detail=f"Speech-to-Text streaming failed: {user_err}.{hint}")
    ...
    *,
    ...
    if bool(getattr(settings, "STT_USE_STREAMING", True)):
        ...
        try:
            ...
            user_err = format_grpc_error_for_user(e)
            raise HTTPException(status_code=502, detail=f"Speech-to-Text recognize failed: {user_err}.{hint}")
