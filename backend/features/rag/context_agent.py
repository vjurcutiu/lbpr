...
def build_context_bundle(
    *,
    ...
    if not clean_query or not files:
        return ContextBundle(query=clean_query, profile=active_profile.name, sufficient=False, chunks=[], retrieval_trace=[], missing_context=["No query or files provided."])
    ...
    sufficient = _has_direct_signal(clean_query, selected)
    round_no = 0
    ...
    missing_context = [] if sufficient else ["Retrieved context may be incomplete for the question."]
