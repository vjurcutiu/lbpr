...
async def ingest(req: IngestRequest, request: Request, user: SessionOut = Depends(get_current_user)):
    try:
        ...
        log.info("usage_upload_tokens_add", uid=user.uid, dataset=req.dataset, tokens=tokens, allowed=ok, used_upload_tokens=used, cap_upload_tokens=cap, client=str(request.client))
        if not ok:
