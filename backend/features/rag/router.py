...
async def ingest(req: IngestRequest, request: Request, user: SessionOut = Depends(get_current_user)):
    try:
        ...
        log.info("ingest_request", dataset=req.dataset, has_text=bool(req.text), meta_keys=list((req.metadata or {}).keys()), client=str(request.client), uid=user.uid, tokens=tokens, used_upload_tokens=used, cap_upload_tokens=cap)
