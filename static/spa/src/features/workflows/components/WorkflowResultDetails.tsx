...
export function WorkflowResultDetails({ result }: { result: WorkflowResult }) {
  ...
  const copyPreview = async () => {
    ...
    try {
      ...
      toast.success("Workflow result copied");
    } catch {
      toast.error("Could not copy the workflow result");
    }
  };
    ...
    <div className="space-y-4">
      {!!warnings.length && (
        <div className="flex flex-wrap gap-2">
          {warnings.map((warning) => (
            <Badge key={warning} variant="outline" className="whitespace-normal text-left text-[11px] text-muted-foreground">
              {warning}
            </Badge>
          ))}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {sourceFiles.map((source, idx) => (
              <Badge key={`${source.file_id || source.name || 'source'}-${idx}`} variant="secondary" className="max-w-full whitespace-normal text-left">
      ...
      {!!fields.length && (
        <div className="space-y-2 rounded-xl border bg-muted/20 p-3">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Extracted fields</div>
          <div className="grid gap-2 sm:grid-cols-2">
            {fields.map((field, idx) => (
              <div key={`${field.field || 'field'}-${idx}`} className="rounded-lg border bg-background p-2">
      ...
      {!!differences.length && (
        <div className="space-y-2 rounded-xl border bg-muted/20 p-3">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Key differences</div>
          <div className="space-y-2">
            {differences.map((difference, idx) => (
              <div key={`${difference.topic || 'difference'}-${idx}`} className="rounded-lg border bg-background p-3">
                ...
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  <div>
                    ...
                    <div className="text-sm">{difference.file_a || "—"}</div>
                  </div>
                  <div>
                    <div className="text-[11px] uppercase tracking-wide text-muted-foreground">File B</div>
                    <div className="text-sm">{difference.file_b || "—"}</div>
                  </div>
                </div>
                {difference.impact ? (
                  <div className="mt-2 text-sm text-muted-foreground">{difference.impact}</div>
                ) : null}
              </div>
            ))}
          </div>
      ...
      {!!planItems.length && (
        <div className="space-y-2 rounded-xl border bg-muted/20 p-3">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Action plan</div>
          <div className="space-y-2">
            {planItems.map((item, idx) => (
              <div key={`${item.action || 'plan-item'}-${idx}`} className="rounded-lg border bg-background p-3">
      ...
      {preview ? (
        <>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Rendered result</div>
            <Button variant="outline" size="sm" onClick={copyPreview}>
              <Copy className="mr-1.5 h-4 w-4" />
              Copy markdown
            </Button>
          </div>
          <div className="max-h-[420px] overflow-auto rounded-xl border bg-background px-4 py-3 text-sm">
