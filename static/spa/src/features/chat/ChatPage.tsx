...
export default function ChatPage() {
  ...
  const onSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (sessionId) await appendMessage(namespace, sessionId, sysMsg);
  ...
  const composerTextClass =
    "text-left placeholder:text-left leading-[1.4] h-12 py-3";
...
function MessageRow({
  ...
  return (
    <div className={`w-full flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`flex items-start gap-3 max-w-[min(80%,720px)] ${
